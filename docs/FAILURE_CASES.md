...
| SGLang starts model compilation but exits with `FileNotFoundError: ninja` | `sglang[all]` did not install the external build runner required by a JIT extension; invoking venv Python by path also does not activate its `PATH`, and resolving its symlink points back to the base interpreter | Pin `ninja==1.13.0`, prepend the unresolved venv executable directory in the launcher, preserve each failed log, verify no CUDA process remains, and rerun the unchanged SGLang matrix |
