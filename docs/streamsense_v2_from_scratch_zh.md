...
|---|---|---|---:|
| `vllm-qwen25-vl-3b` | vLLM | FP16/BF16 基线权重 | 8001 |
| `vllm-qwen25-vl-3b-fp8` | vLLM | 动态 FP8 | 8002 |
| `sglang-qwen25-vl-3b` | SGLang | FP16/BF16 基线权重 | 30000 |
...
本节在远端三组 profile 全部结束后，从
`matrix_summary.json`、每组 `quality.json`、原始并发报告和 backend log
自动写入。必须同时呈现失败率、TTFT、TPOT、真实 completion tokens/s、
请求吞吐、显存和质量 pass rate；不允许只挑最快的一行。
